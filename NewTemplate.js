document.addEventListener('DOMContentLoaded', () => {
  ...
  function createTreeItem(type, name = '') {
    ...
    // Add empty sub-tree for folders
    if (type === 'folder') {
    ...
    content.addEventListener('click', (e) => {
      e.stopPropagation();
      if (selectedItem) selectedItem.querySelector('.tree-item-content').classList.remove('selected');
      selectedItem = li;
  ...
  function serializeTree(ul) {
    ...
    for (const li of items) {
      ...
      if (type === 'folder') {
        ...
        obj.content = serializeTree(subTree);
      }
